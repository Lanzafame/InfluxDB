...
// NewNodeProcessor returns a new NodeProcessor for the given node, using dir
...
// process attempts to send any existing hinted handoff data to the target node.
func (n *NodeProcessor) process() {
	defer n.wg.Done()
	...
	for {
		...
		case <-time.After(currInterval):
			if nio, err := n.metastore.Node(n.nodeID); err != nil {
